...
def produce_graph_out(graph, original_img):
...
def simplify_junctures_out(graph, original_img):
  out = original_img.copy()
  for node in graph.nodes_iter():
    degree = nx.degree(graph, node)
    if degree != 2:
...
def simplify_paths_out(graph, original_img):
  out = original_img.copy()
...
def straighten_lines_out(graph, original_img):
  out = original_img.copy()
  for (p1, p2) in graph.edges_iter():
    cv2.line(out, p1, p2, (0,255,0), 2)
  for node in graph.nodes_iter():
